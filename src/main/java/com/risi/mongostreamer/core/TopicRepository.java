...
public abstract class TopicRepository<T extends MongoStreamerEntity> {
    ...
    @Value("${mongo.streamer.id}")
    private String mongoStreamerId;
    @Value("${mongo.streamer.start.cleanup}")
    ...
    public Flux<T> findAll(String topic) {
        ...
        return (Flux<T>) template
                .tail(new Query(), entityClass, topic + extensionName)
